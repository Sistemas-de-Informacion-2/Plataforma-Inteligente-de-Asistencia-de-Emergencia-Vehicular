// src/app/pages/sucursales/ta-reportes/ta-reportes.component.ts
...
@Component({
  selector: 'app-ta-reportes',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './ta-reportes.component.html',
  styles: [`
    .shimmer {
      background: #f6f7f8;
...
export class TaReportesComponent {
  private svc = inject(ReportesService);

  // ── KPIs disponibles para TA (incluye KPI 11 — ranking de mecánicos)
  // Excluye KPI 6 (ranking global de talleres) y KPI 7 (mapa global) — SA only
  readonly kpiCards: KpiCard[] = KPI_CARDS.filter(k => k.id !== 6 && k.id !== 7);
