...
import 'package:fixo/features/emergencias/screens/tracking_screen.dart';

import 'package:fixo/features/emergencias/widgets/emergencia_map_widget.dart';
import 'package:fixo/features/emergencias/widgets/map_circle_button.dart';
import 'package:fixo/features/emergencias/widgets/vehicle_pill_selector.dart';
import 'package:fixo/features/emergencias/widgets/attach_option_tile.dart';
import 'package:fixo/features/emergencias/widgets/sos_bottom_bar.dart';
import 'package:fixo/features/emergencias/widgets/recommendations_bottom_sheet.dart';
...
class _InicioScreenState extends State<InicioScreen> {
  ...
  void _onEmergenciaStateChanged() {
    ...
    } else if (flowState == EmergenciaFlowState.rejected || flowState == EmergenciaFlowState.timeout) {
       ScaffoldMessenger.of(context).showSnackBar(
         SnackBar(content: Text(_emProvider!.errorMessage), backgroundColor: AppTheme.danger),
       );
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              AttachOptionTile(
                ...
                onTap: () { Navigator.pop(ctx); provider.pickFromCamera(); },
              ),
              ...
              AttachOptionTile(
                ...
                onTap: () { Navigator.pop(ctx); provider.pickImages(); },
              ),
              const SizedBox(height: 8),
            ],
          ),
  ...
  Future<void> _enviarSOS() async {
    ...
    if (result != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(emProvider.errorMessage.isNotEmpty
  ...
  void _showRecommendationsSheet(EmergenciaProvider emProvider) {
    showModalBottomSheet(
      ...
      builder: (_) => ListenableBuilder(
        ...
        builder: (context, _) {
          if (emProvider.recomendaciones == null) return const SizedBox.shrink();
          return RecommendationsBottomSheet(
            result: emProvider.recomendaciones!,
            isSending: emProvider.flowState == EmergenciaFlowState.sendingSelection,
            ...
            onSelectShop: (sucursal) async {
              ...
              if (!success && context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(emProvider.errorMessage), backgroundColor: AppTheme.danger),
                );
              }
            },
          );
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // ── 4. HEADER FLOTANTE ────────────────────────────
          Positioned(
            top: mq.padding.top + 12, left: 16, right: 16,
            child: Row(
              children: [
                MapCircleButton(
